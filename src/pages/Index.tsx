...
const Index = () => {
  ...
  const { calculateStats, loading, clearAllData, generateReport } = useAppContext();
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">
                {userPlan.currentPlan.maxClients === null 
                  ? "Clientes ilimitados" 
                  : `${userPlan.clientCount}/${userPlan.currentPlan.maxClients} clientes`}
              </p>
              <p className="text-lg font-semibold text-primary">
                {userPlan.currentPlan.price === 0 
                  ? "Gratuito" 
                  : `R$ ${userPlan.currentPlan.price.toFixed(2)}/mês`}
              </p>
            </div>
            <Button
              variant="outline"
